...
sudo -u ${DEFAULT_POSTGRES_USER} psql -d poc_data_pipelines -c 'create table if not exists kafka_spark_streaming_pipeline.raw_trades ( trade_id VARCHAR(15) NOT NULL, product_id VARCHAR(15) NOT NULL, price REAL NOT NULL, size REAL NOT NULL, time TIMESTAMP WITH TIME ZONE NOT NULL, side VARCHAR(5) NOT NULL, bid REAL, ask REAL, api_call_timestamp_utc TIMESTAMP WITHOUT TIME ZONE NOT NULL );'
sudo -u ${DEFAULT_POSTGRES_USER} psql -d poc_data_pipelines -c 'create table if not exists kafka_spark_streaming_pipeline.raw_trades_deduped ( trade_id VARCHAR(15) NOT NULL, product_id VARCHAR(15) NOT NULL, price REAL NOT NULL, size REAL NOT NULL, time TIMESTAMP WITH TIME ZONE NOT NULL, side VARCHAR(5) NOT NULL, bid REAL, ask REAL, api_call_timestamp_utc TIMESTAMP WITHOUT TIME ZONE NOT NULL, CONSTRAINT raw_table_deduped_pk PRIMARY KEY (trade_id) );'
